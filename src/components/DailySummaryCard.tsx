...
export default function DailySummaryCard({ agent, tasks, compliance }: Props) {
  ...
  const lastActive = agent.last_active ? relativeTime(agent.last_active) : 'Unknown'
  ...
  return (
    <div style={{ background: 'var(--bt-surface)', border: '1px solid var(--bt-border)', borderRadius: 6, padding: '20px 24px' }}>
      ...
      <div style={{ marginBottom: 20, paddingBottom: 16, borderBottom: '1px solid var(--bt-border)' }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--bt-accent)' }}>
          {getPhaseLabel(agent.onboarding_stage ?? 0)}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        ...
        <StatBox label="Compliance Gaps" value={complianceGaps.length} color={complianceGaps.length > 0 ? 'var(--bt-red)' : 'var(--bt-green)'} />
      </div>
