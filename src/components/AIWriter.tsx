...
export default function AIWriter({ open, onClose }: AIWriterProps) {
  ...
  useEffect(() => {
    if (open) {
      // Auto-fill from selected pipeline lead
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              color: 'var(--bt-text-dim)', fontSize: 20,
        ...
        <div style={{ padding: '14px 20px 0', flexShrink: 0 }}>
          <div style={{ fontSize: 10, color: 'var(--bt-text-dim)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 8, fontWeight: 600 }}>
            Quick Templates
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {QUICK_PROMPTS.map((p) => (
              <button
                ...
                disabled={loading}
                className="ai-writer-prompt-btn"
                style={{
        ...
        <div style={{ padding: '10px 20px 0', flexShrink: 0 }}>
          <div style={{ fontSize: 10, color: 'var(--bt-text-dim)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 4, fontWeight: 600 }}>
            Client Name
          </div>
          <input
            value={clientName}
            ...
            style={{
              ...
              color: 'var(--bt-text)', borderRadius: 5, outline: 'none',
              fontFamily: 'inherit',
            }}
          />
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ padding: '10px 20px 0', flexShrink: 0 }}>
          <div style={{ fontSize: 10, color: 'var(--bt-text-dim)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 6, fontWeight: 600 }}>
            Custom Prompt
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <textarea
              ...
              placeholder="e.g. Write a follow-up text to a buyer who toured 3 homes yesterday..."
              rows={3}
              onKeyDown={e => {
              ...
              style={{
                ...
                background: 'var(--bt-surface)', border: '1px solid var(--bt-border)',
                color: 'var(--bt-text)', borderRadius: 5, resize: 'none',
            ...
            <button
              ...
              style={{
                padding: '0 16px',
                background: input.trim() && !loading ? 'var(--bt-accent)' : 'var(--bt-border)',
        ...
        <div style={{ flex: 1, padding: '14px 20px', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          <div style={{ fontSize: 10, color: 'var(--bt-text-dim)', letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 8, flexShrink: 0 }}>
            Output
          </div>

          {loading ? (
            <div style={{
              flex: 1, background: 'var(--bt-surface)', border: '1px solid var(--bt-border)',
              borderRadius: 5, padding: '12px 14px', fontSize: 13, lineHeight: 1.7,
              color: 'var(--bt-text-dim)', fontStyle: 'italic', minHeight: 120,
            }}>Writing...</div>
          ) : output ? (
            <textarea
              value={output}
              ...
              style={{
                flex: 1, background: 'var(--bt-surface)', border: '1px solid var(--bt-border)',
                borderRadius: 5, padding: '12px 14px', fontSize: 13, lineHeight: 1.7,
                color: 'var(--bt-text)', resize: 'none', outline: 'none',
                fontFamily: 'inherit', minHeight: 120, whiteSpace: 'pre-wrap',
              }}
            />
          ) : (
            <div style={{
              ...
              borderRadius: 5, padding: '12px 14px', fontSize: 13, lineHeight: 1.7,
              color: 'var(--bt-text-dim)', fontStyle: 'italic', minHeight: 120,
            }}>Your email will appear here</div>
          )}
              ...
              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const res = await fetch('/api/send-email', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ to: recipientEmail.trim(), subject, body: emailBody, fromName: agentName, fromEmail: agentEmail }),
                      })
                      if (res.ok) {
                        setSendStatus('Sent!')
                  ...
                  style={{
                    ...
                    background: recipientEmail.trim() && !sending ? '#E04E4E' : 'var(--bt-border)',
                    border: 'none', color: '#fff', borderRadius: 4,
                    cursor: recipientEmail.trim() ? 'pointer' : 'default', whiteSpace: 'nowrap',
                  }}
                >
                  {sending ? 'Sending...' : sendStatus || 'Send'}
                </button>
                <button
                  onClick={copyOutput}
                  style={{
                    fontSize: 12, padding: '8px 12px',
                    background: copied ? 'var(--bt-accent)' : 'var(--bt-surface)',
                    border: '1px solid var(--bt-border)',
                    color: copied ? 'var(--bt-black)' : 'var(--bt-text-dim)',
                    borderRadius: 4, cursor: 'pointer', fontWeight: copied ? 700 : 400,
                  }}
                >
                  {copied ? 'Copied' : 'Copy'}
                </button>
