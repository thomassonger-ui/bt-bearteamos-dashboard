...
const ACTION_LABEL: Record<string, string> = {
  ...
  pipeline_contact: 'Pipeline Contact',
  compliance_completed: 'Compliance Completed',
  login: 'Session Started',
  engine_rule_triggered: 'Engine Rule Triggered',
}
...
export default function MemoryLog({ log }: Props) {
  ...
  return (
    <div style={{ background: 'var(--bt-surface)', border: '1px solid var(--bt-border)', borderRadius: 6 }}>
      <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--bt-border)', display: 'flex', justifyContent: 'space-between' }}>
        <div style={{ fontSize: 11, color: 'var(--bt-text-dim)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Memory Log</div>
        ...
        {sorted.map((entry, i) => (
          <div key={entry.id} style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--bt-text)', marginBottom: 2 }}>{entry.description}</div>
              <div style={{ fontSize: 11, color: 'var(--bt-text-dim)' }}>{formatDate(entry.created_at)}</div>
            </div>
          </div>
        ))}
      </div>
